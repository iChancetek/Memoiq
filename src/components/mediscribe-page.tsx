...
import { useMediScribe, type ScribeEntry } from '@/contexts/mediscribe-context';
...
function Recorder({ onNewRecording, disabled }: { onNewRecording: (audioBlob: Blob) => void, disabled: boolean }) {
    ...
    return (
        <Card>
            <CardHeader>
                <CardTitle>MediScribe</CardTitle>
...
function RecordingItem({ recording, onDelete }: { recording: ScribeEntry, onDelete: (id: string) => void }) {
    const { translateScribeEntry } = useMediScribe();
...
export function MediScribePage() {
